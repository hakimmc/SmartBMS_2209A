...
/*      Test Messages Blocks Start   */

/*      Test Messages Blocks End     */
...
void BQ_Test()
{
    ...
    //while(Uart_Receive(BQ_UART_PORT, temp_data, 4, 1000) < 0);
}
...
void BQ_Start(void *args){
    ...
    while (1) {
        ...
        if (BQ_Uart_Transmit(BQ_UART_PORT, data, 4)) {
            printf("Data sent\n");
        } else {
...
uint8_t* CRC16(uint8_t* data, uint8_t data_length)
{
    ...
    out_data[0] = (uint8_t) ((CRC >> 8) & 0xFF);
    out_data[1] = (uint8_t) (CRC & 0xFF);
    return out_data;
}
